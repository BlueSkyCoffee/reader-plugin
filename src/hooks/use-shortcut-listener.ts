...
import { eventToKeyCombo, normalizeKeyCombo, useShortcuts } from "./use-shortcuts"
...
export function useShortcutListener(
  ...
  const handleKeyDown = useCallback(
    (event: KeyboardEvent) => {
      ...
      // 检查按键组合是否匹配
      if (JSON.stringify(keys) === JSON.stringify(normalizeKeyCombo(shortcut.keys))) {
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (event: KeyboardEvent) => {
      ...
      for (const [shortcutId, callback] of Object.entries(shortcuts)) {
        ...
        if (JSON.stringify(pressedKeys) === JSON.stringify(normalizeKeyCombo(shortcut.keys))) {
          event.preventDefault()
          callback()
          break
        }
