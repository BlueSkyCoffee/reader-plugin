import type { LightNovelSource } from "./sources-base"
  ...
  private async getShuffleParams(doc: Document): Promise<ShuffleParams | null> {
    return lock.run(async () => {
      ...
      if (currentVersion !== matchedVersion && !warnedVersionMismatch) {
        ...
        console.warn(`[Bili] chapterlog version mismatch: ${currentVersion} vs ${matchedVersion}, may cause chapter order issues`)
      }
      if (!chapterId) {
        return null
      }
