...
function shuffleContent(content: Element, params: ShuffleParams) {
  ...
  const mapped = Array.from<Element | null>({ length: pElements.length }).fill(null)
...
export class BiliNovelParser implements INovelParser {
  async parse(input: string): Promise<LightNovelInfo> {
    const novel = await parseBilibiliNovel(input)
